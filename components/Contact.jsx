...
const Contact = ({isDarkMode}) => {
  ...
  const onSubmit = async (event) => {
    ...
    const api_data  = process.env.NEXT_PUBLIC_WEB_CONTACT;
    formData.append("access_key", api_data);
    // formData.append("access_key", "fb240a2f-b0a6-4bc0-892c-caa8969c23c7");
    ...
    id='contact' className="w-full px-[12%] py-10 scroll-mt-20 bg-[url('/footer-bg-color.png')] bg-no-repeat bg-center bg-[length:90%_auto] dark:bg-none">
       ...
       onSubmit={onSubmit}  className='max-w-2xl mx-auto'>
        <div className='grid grid-cols-auto gap-6 mt-10 mb-6'>
            ...
            type="text" placeholder='Enter your email' required className='flex-1 p-3 outline-none border-[0.5px] border-gray-400 rounded-md bg-white dark:bg-darkHover/30 dark:border-white/90' name='email'/>
        </div> 
        <motion.textarea 
